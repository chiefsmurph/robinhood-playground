  ...
  const withTSO = withoutLowVolume
    .map(buy => ({
      ...
      computed: {
        ...
        tsh: getTrend(buy.quote.currentPrice, buy.fundamentals.low),
      }
    }))
    .map(buy => ({
      ...buy,
...
const calcZscoreOffset = buy => {
  ...
  const recentPickTrendOffset = sumArray([
    ...
    recent500TrendPerDay < -4.5 && Math.min(15, Math.abs(Math.round(recent500TrendPerDay * 2)))
  ]);
  ...
  const lowerTscOffset = tsc < -40 && (Math.abs(tsc) - 40) * 1.6;
  const upperTscOffset = tsc > 0 && tsc * -2.5;
  ...
  const numOffsets = Object.values(offsets).filter(off => off > 0).length;
