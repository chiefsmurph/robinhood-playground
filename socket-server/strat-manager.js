...
const stratManager = {
    ...
    async resetPositionWatchers() {
        console.log(
            this.positions.alpaca.sort((a, b) => Number(b.equity) - Number(a.equity)).map(({ ticker, equity }) => ({ ticker, equity })),
            'hey over heres'
        );
        for (let pos of this.positions.alpaca.sort((a, b) => Number(b.equity) - Number(a.equity))) {
