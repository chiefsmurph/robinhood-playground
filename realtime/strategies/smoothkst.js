...
const smoothKST = kstSeries => {
    ...
    const allKST = kstSeries.map(obj => obj.kst);
    const smoothSignals = EMA.calculate({
        values: allKST,
        period: 15
    });
    return kstSeries.map(({ kst, signal }, index, arr) => ({
        kst: smoothSignals[ smoothSignals.length - (arr.length - index) ],
        signal,
    }));
};
